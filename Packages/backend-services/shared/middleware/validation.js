// packages/backend-services/shared/middleware/validation.js
...
class ValidationMiddleware {
  ...
  schemas = {
    ...
    password: Joi.string().min(8).pattern(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]/).messages({
        'string.pattern.base': 'Password must contain at least 8 characters with uppercase, lowercase, number, and a special character',
        'string.min': 'Password must be at least 8 characters long'
    }),
    
    // Mobile number (Indian format)
    mobileNumber: Joi.string().pattern(/^[6-9]\d{9}$/).messages({
